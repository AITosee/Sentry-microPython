...
while True:
    ts = tn;
    obj_num = sentry.GetValue(sentry_vision_e.kVisionCard, kStatus)
    tn = time.ticks_ms()
    if obj_num:
        print("Totally %d objects in %dms:"%( obj_num, tn - ts))
        for i in range(obj_num):
            x = sentry.GetValue(sentry_vision_e.kVisionCard, kXValue)
            y = sentry.GetValue(sentry_vision_e.kVisionCard, kYValue)
            w = sentry.GetValue(sentry_vision_e.kVisionCard, kWidthValue)
            h = sentry.GetValue(sentry_vision_e.kVisionCard, kHeightValue)
            l = sentry.GetValue(sentry_vision_e.kVisionCard, kLabel, i)
